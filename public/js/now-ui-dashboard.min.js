...
toggle_initialized = sidebar_mini_active = backgroundOrange = navbar_initialized = fixedTop = !(
    transparentDemo = transparent = !0
);
var is_iPad = null != navigator
        .userAgent
        .match(/iPad/i),
    scrollElement = -1 < navigator
        .platform
        .indexOf("Win")
            ? $(".main-panel")
            : $(window);
...
$(window).resize(function () {
    ...
    0 == $(".full-screen-map").length && 0 == $(".bd-docs").length && (
        ...
        $navbar.hasClass("bg-white") && 991 < $(window).width()
            ? 0 == scrollElement.scrollTop() && $navbar.removeClass("bg-white").addClass(
                "navbar-transparent"
            )
            : $navbar.hasClass("navbar-transparent") && $(window).width() < 991 && "false" != isExpanded && $navbar.addClass(
                "bg-white"
            ).removeClass("navbar-transparent")
    ),
        ...
        $.notify(
            {
                ...
                message: "Welcome to <b>Now Ui Dashboard</b> - a beautiful freebie for every web develop" +
                        "er."
            },
            {
                type: color,
